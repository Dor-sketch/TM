...
Proof: Suppose there is a TM $H$ that decides $E_{TM}$. We can use $H$ to decide $A_{TM}$ as follows:
...
4. If $H$ accepts $<M'>$ then accept, otherwise reject.

$H$ decides $E_{TM}$, but $E_{TM}$ is undecidable, so $H$ cannot exist.
...
Proof: The complement of $E_{TM}$ is recognizable because we can use the same TM $H$ for $E_{TM}$ and just change the accept/reject conditions.
...
Proof: Use the diagonalization method to prove that $HALT_{TM}$ is undecidable. Suppose there is a TM $H$ that decides $HALT_{TM}$. We can Construct a word $w$ that is not in $HALT_{TM}$ as follows:

1. Enumerate all strings $w_1, w_2, w_3, ...$ in $\Sigma^*$.

2. For each $i$, run $H$ on $<M_i, w_i>$, where $M_i$ is the $i$th TM in the enumeration.

3. If $H$ accepts $<M_i, w_i>$, then $w_i$ is in $HALT_{TM}$.

4. If $H$ rejects $<M_i, w_i>$, then $w_i$ is not in $HALT_{TM}$.

5. Construct a word $w$ that is not in $HALT_{TM}$ by taking the $i$th character of $w_i$ and changing it to a different character.

6. $H$ cannot decide whether $w$ is in $HALT_{TM}$, because $w$ is constructed to be different from all $w_i$.

7. Therefore, $HALT_{TM}$ is undecidable.
